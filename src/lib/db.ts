...
export function getDatabase() {
  ...
  if (typeof self !== 'undefined' && 'D1_DATABASE' in self) {
    // @ts-expect-error - self.D1_DATABASE in worker environment
    db = self.D1_DATABASE;
    if (db) {
...
export class HTBMachinesDB {
  ...
  async createMachine(machine: any): Promise<any | null> {
    ...
    try {
      const prepared = await this.db.prepare(`
        ...
        (id, name, os, difficulty, status, date_completed, tags, writeup, created_at, updated_at)
  ...
  async updateMachine(id: string, machine: any): Promise<any | null> {
    ...
    try {
      const prepared = await this.db.prepare(`
        UPDATE htb_machines 
        SET name = ?, os = ?, difficulty = ?, status = ?, date_completed = ?, 
