...
// Fetch and get latest completed machines with writeups
  useEffect(() => {
    const fetchLatestMachines = async () => {
      try {
        const response = await fetch('/api/admin/htb-machines-d1');
        const data = await response.json();
        if (response.ok) {
          const machinesList: Machine[] = Array.isArray(data) ? data : (data.machines || []);

          // Get latest completed machines sorted by completion date and must have writeups
          const completedMachinesWithWriteups = machinesList
            .filter(machine => machine.status === 'Completed' && machine.dateCompleted && machine.writeup)
            .sort((a, b) => {
          ...
          setLatestMachines(completedMachinesWithWriteups);
        }
      } catch (error) {
    ...
    // Fetch periodically for updates
    const interval = setInterval(fetchLatestMachines, 5000);
