...
const MachineCard = ({ machine, isAdminMode, onViewWriteup, onEdit, onDelete }: MachineCardProps) => {
  const getOSIcon = () => {
    ...
    <div className="bg-card-bg border border-cyber-green/30 rounded-lg p-6 hover:border-cyber-green transition-all duration-300 group">
      ...
      <div className="flex justify-between items-center">
        <button
          ...
          disabled={!machine.writeup}
          className={`flex items-center space-x-2 px-4 py-2 rounded font-semibold transition-all duration-300 border-2 shadow-md ${
            machine.writeup
              ? 'bg-cyber-green text-white hover:bg-cyber-blue hover:text-white hover:shadow-xl border-cyber-green hover:border-cyber-blue transform hover:scale-105'
              : 'bg-gray-800 text-gray-200 cursor-not-allowed border-gray-500 opacity-75'
          }`}
        >
          <FaEye className={machine.writeup ? 'text-white' : 'text-gray-400'} />
